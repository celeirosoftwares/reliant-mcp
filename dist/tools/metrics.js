...
exports.metricsTools = void 0;
exports.metricsTools = [
    {
    ...
    },
    {
        ...
        handler: async (client, _args) => {
            ...
            const slaBreaches = (slas?.slas || []).filter((s) => s.sla_status === 'breached');
            const lines = [
                ...
                `**SLA breaches:** ${slaBreaches.length > 0 ? `🚨 ${slaBreaches.length} breach(es): ${slaBreaches.map((s) => s.schema_id).join(', ')}` : '✅ None'}`,
            ];
            return lines.join('\n');
        },
    },
