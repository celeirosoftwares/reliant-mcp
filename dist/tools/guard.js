...
exports.guardTools = [
    ...
    },
    {
        ...
        handler: async (client, _args) => {
            ...
            if (configs.length === 0)
                return 'No Guard configs found. Create one with reliant_create_guard.';
            const lines = configs.map((g) => `- **${g.name}** (ID: \`${g.id}\`) | ${g.provider}/${g.model} | ${g.is_active ? '🟢 active' : '🔴 inactive'}`);
    ...
    },
    {
        ...
        handler: async (client, args) => {
            ...
            const lines = [
                ...
                '',
                '**Top end-users:**',
                ...(data.top_users || []).slice(0, 10).map((u) => `- \`${u.end_user_id}\`: ${u.count} executions`),
            ];
            return lines.join('\n');
        },
    },
