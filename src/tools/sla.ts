...
export const slaTools = [
  {
    ...
    handler: async (client: ReliantClient, args: any) => {
      const result = await client.get(
        `/sla/${args.schema_id}/evaluate?user_id=${client.userId}`
      )
      ...
      const lines = [
        ...
        result.breaches?.length
          ? `**⚠️ Breaches detected:**\n${result.breaches.map((b: string) => `- ${b}`).join('\n')}`
...

  {
    ...
    handler: async (client: ReliantClient, _args: any) => {
      ...
      if (slas.length === 0) return 'No SLAs configured. Use reliant_sla_configure to set one up.'

      const statusIcon = (s: string) => s === 'healthy' ? '✅' : s === 'degraded' ? '⚠️' : s === 'breached' ? '🚨' : '❓'

      const lines = slas.map((s: any) =>
        `${statusIcon(s.sla_status)} **${s.schema_id}** | target: ${s.target_success_rate}% | current: ${s.current_success_rate?.toFixed(1) ?? '—'}% | ${s.sla_status}`
      )
